  ...
  return (
    ...
    >
      ...
      <div className="mt-4 flex flex-1 flex-col sm:mt-5">
        <h2
          className={[
            "text-[1.08rem] font-bold tracking-[-0.03em] transition group-hover:text-sky-700 sm:text-[1.28rem] md:text-[1.38rem]",
            light ? "text-slate-900" : "text-slate-50 group-hover:text-cyan-200"
          ].join(" ")}
        >
          <Link href={`/${locale}/blog/${article.slug}`}>{article.title}</Link>
        </h2>
        <p className={["mobile-clamp-2 mt-3 text-[14px] leading-6 sm:mt-4 sm:text-sm sm:leading-7 md:text-[15px] md:leading-8", light ? "text-slate-600" : "text-slate-300/84"].join(" ")}>
          {article.excerpt}
        </p>

        <div className={["mt-auto flex items-center justify-between gap-3 border-t pt-4 text-sm sm:mt-6 sm:gap-4 sm:pt-5", light ? "border-slate-200" : "border-sky-400/10"].join(" ")}>
          <span className={["truncate", light ? "text-slate-500" : "text-slate-400"].join(" ")}>

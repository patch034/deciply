  ...
  return (
    <section className="relative mx-auto w-full max-w-[1200px] px-4 pt-3 sm:px-6 sm:pt-5 lg:pt-7">
      <div className="pointer-events-none absolute inset-x-0 top-0 -z-10 h-[34rem] overflow-hidden">
        ...
        />
        <motion.div
          className="absolute right-[5%] top-0 h-80 w-80 rounded-full bg-sky-500/22 blur-3xl"
          animate={{ opacity: [0.34, 0.72, 0.34], scale: [1, 1.1, 1] }}
          transition={{ duration: 10, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut" }}
        />
        <div className="absolute left-1/2 top-24 h-80 w-80 -translate-x-1/2 rounded-full bg-fuchsia-400/14 blur-3xl" />
      </div>

      <GlassPanel className="relative overflow-hidden rounded-[32px] border-violet-400/14 bg-[linear-gradient(180deg,rgba(8,11,22,0.92),rgba(13,18,34,0.98))] px-4 py-4 shadow-[0_28px_96px_-56px_rgba(124,58,237,0.24),0_18px_64px_-48px_rgba(244,114,182,0.14)] sm:px-6 sm:py-5">
        <div className="absolute inset-x-12 top-0 h-36 rounded-full bg-violet-500/18 blur-3xl" />
        <div className="absolute -right-24 top-10 h-56 w-56 rounded-full bg-fuchsia-400/14 blur-3xl" />
        ...
        <div className="relative grid gap-7 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:gap-8">
          <div>
            ...
            <motion.h1
              className="balance-text mt-4 max-w-4xl bg-gradient-to-r from-white via-violet-200 to-fuchsia-200 bg-clip-text pb-2 text-[28px] font-extrabold leading-[1.14] tracking-[-0.05em] text-transparent sm:mt-6 sm:text-5xl lg:text-[3.9rem] lg:leading-[1.08]"
            ...
            >
              {quickLinks.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="inline-flex min-h-[40px] items-center rounded-full border border-violet-400/14 bg-white/[0.03] px-4 text-sm font-semibold text-slate-200 transition duration-200 hover:border-fuchsia-400/24 hover:text-fuchsia-100"
                >
                  {item.label}
                </Link>
              ))}
            ...
            <motion.p
              className="mt-3 text-sm font-medium leading-6 text-fuchsia-100/90"
              initial={{ opacity: 0, y: 12 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            {hasStarterHint ? (
              ...
              >
                ...
                <Link href={`/${locale}${content.starterHintHref}`} className="inline-flex items-center gap-2 font-semibold text-fuchsia-100 transition duration-200 hover:text-fuchsia-50">
            ...
            <div className="ui-card-strong rounded-[28px] p-3 text-white sm:p-5">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-xs font-medium uppercase tracking-[0.18em] text-fuchsia-100/78">{content.panelEyebrow}</p>

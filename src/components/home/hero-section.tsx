  ...
  return (
    <section className="relative mx-auto w-full max-w-[1200px] px-4 pt-3 sm:px-6 sm:pt-5 lg:pt-7">
      <div className="pointer-events-none absolute inset-x-0 top-0 -z-10 h-[38rem] overflow-hidden">
        <motion.div
          className="absolute left-[2%] top-10 h-80 w-80 rounded-full bg-fuchsia-500/30 blur-3xl"
          animate={{ opacity: [0.42, 0.82, 0.42], scale: [1, 1.08, 1] }}
          transition={{ duration: 9, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute right-[5%] top-0 h-[22rem] w-[22rem] rounded-full bg-sky-500/24 blur-3xl"
          animate={{ opacity: [0.34, 0.72, 0.34], scale: [1, 1.1, 1] }}
          transition={{ duration: 10, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut" }}
        />
        <div className="absolute left-1/2 top-24 h-[22rem] w-[22rem] -translate-x-1/2 rounded-full bg-fuchsia-400/12 blur-3xl" />
      </div>

      <GlassPanel className="relative overflow-hidden rounded-[34px] border border-violet-400/14 bg-[linear-gradient(180deg,rgba(3,6,12,0.96),rgba(12,18,34,0.98))] px-4 py-4 shadow-[0_30px_94px_-56px_rgba(6,10,18,0.86)] sm:px-6 sm:py-5">
        <div className="absolute inset-x-12 top-0 h-40 rounded-full bg-fuchsia-500/18 blur-3xl" />
        <div className="absolute -right-24 top-10 h-60 w-60 rounded-full bg-cyan-400/12 blur-3xl" />
        <div className="absolute left-20 bottom-0 h-32 w-32 rounded-full bg-fuchsia-500/10 blur-3xl" />
        ...
        <div className="relative grid gap-7 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:gap-8">
          <div>
            ...
            <motion.h1
              className="balance-text mt-4 max-w-4xl bg-gradient-to-r from-white via-sky-200 via-violet-200 to-fuchsia-300 bg-clip-text pb-2 text-[28px] font-extrabold leading-[1.14] tracking-[-0.05em] text-transparent sm:mt-6 sm:text-5xl lg:text-[3.9rem] lg:leading-[1.08]"
            ...
            >
              {quickLinks.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="inline-flex min-h-[40px] items-center rounded-full border border-violet-400/18 bg-slate-950/60 px-4 text-sm font-semibold text-slate-200 transition duration-200 hover:border-fuchsia-400/25 hover:bg-fuchsia-400/[0.08] hover:text-fuchsia-100"
                >
                  {item.label}
                </Link>
              ))}
            ...
            <motion.p
              className="mt-3 text-sm font-medium leading-6 text-fuchsia-100/92"
              initial={{ opacity: 0, y: 12 }}
              animate={{ opacity: 1, y: 0 }}

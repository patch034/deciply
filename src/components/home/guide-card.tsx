  ...
  return (
    <motion.div whileHover={{ y: -8, scale: 1.012 }} transition={{ duration: 0.22 }} className="h-full">
      <Link href={`/${locale}${item.href}`} className="group block h-full">
        <GlassPanel className="home-card-glow flex h-full min-h-[300px] flex-col overflow-hidden rounded-[30px] border-sky-400/10 bg-[linear-gradient(160deg,rgba(9,14,27,0.96),rgba(5,9,17,0.99))] p-7 shadow-[0_28px_90px_-42px_rgba(6,10,18,0.72)] transition duration-300 hover:border-cyan-400/22 hover:shadow-[0_34px_98px_-40px_rgba(14,165,233,0.14)] md:p-8">
          <div className="flex items-center justify-between gap-3">
            <div className="flex min-w-0 items-center gap-3">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-sky-500 via-blue-500 to-cyan-400 text-sm font-bold uppercase tracking-[0.16em] text-white shadow-[0_16px_36px_-18px_rgba(37,99,235,0.3)]">
                {item.icon}
              </div>
          ...
          <h3 className="mt-5 text-[1.25rem] font-bold tracking-[-0.03em] text-slate-50 sm:text-[1.55rem]">{item.title}</h3>
          <p className="mt-3 flex-1 text-[15px] leading-7 text-slate-300/88 sm:text-base sm:leading-8">{item.description}</p>

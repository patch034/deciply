...
export function DiscoveryToolCard({ locale, tool, ctaLabel, compact = false }: DiscoveryToolCardProps) {
  ...
  return (
    <motion.div whileHover={{ y: -4, scale: 1.01 }} transition={{ duration: 0.22 }} className="h-full">
      <div className="group relative flex h-full flex-col overflow-hidden rounded-[28px] border border-slate-200/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(248,250,252,0.98))] p-5 shadow-[0_24px_72px_-42px_rgba(15,23,42,0.16)] transition duration-300 hover:-translate-y-1 hover:border-sky-200 hover:shadow-[0_34px_88px_-44px_rgba(37,99,235,0.16)] sm:p-6">
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-[18px] bg-[var(--tn-gradient-primary)] text-sm font-bold uppercase tracking-[0.14em] text-white shadow-[0_16px_36px_-18px_rgba(37,99,235,0.32)]">
              {tool.icon}
            </div>
            <div className="min-w-0">
              <p className="truncate text-xs font-medium uppercase tracking-[0.16em] text-slate-500">{tool.category}</p>
        ...
        <Link
          ...
          className="mt-5 block min-w-0"
        >
          <h3 className="clamp-2 text-[1.08rem] font-bold tracking-[-0.03em] text-slate-950 transition duration-200 group-hover:text-sky-700 sm:text-[1.32rem]">
            {tool.name}
          </h3>
        </Link>

  ...
  return (
    <motion.div whileHover={{ y: -4, scale: 1.012 }} transition={{ duration: 0.22 }} className="h-full">
      <Link href={href} className="group block h-full">
        <GlassPanel className={`home-card-glow flex h-full flex-col justify-between overflow-hidden p-5 sm:p-6 ${featured ? "ui-card-strong ui-card-hover" : "ui-card ui-card-hover"}`}>
          ...
          <div className="flex flex-1 flex-col">
            <h3 className="clamp-2 mt-3 min-h-[2.4rem] text-[1.08rem] font-bold leading-tight tracking-[-0.03em] text-slate-50 sm:mt-5 sm:min-h-[3rem] sm:text-[1.34rem]">{item.title}</h3>
            <p className="mobile-clamp-2 mt-2.5 flex-1 text-[14px] leading-relaxed text-slate-300/84 sm:mt-3.5 sm:text-[15px]">{item.description}</p>
          </div>

          <div className="mt-auto border-t border-sky-400/10 pt-3.5 sm:mt-6 sm:pt-5">

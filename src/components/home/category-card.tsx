...
export function CategoryCard({ locale, category, linkLabel, tone = "light" }: CategoryCardProps) {
  ...
  return (
    <Link href={`/${locale}${category.href}`} className="group block h-full">
      <article
        className={[
          "ui-card ui-card-hover home-card-glow flex h-full flex-col overflow-hidden rounded-[24px] p-[18px] transition duration-300 hover:-translate-y-1 sm:p-[18px] md:p-5",
          light
      ...
      >
        ...
        <p className={["mt-4 text-xs font-medium uppercase tracking-[0.18em]", light ? "text-slate-500" : "text-slate-400"].join(" ")}>
          {category.eyebrow}
        </p>
        <h3
          className={[
          "clamp-2 mt-2 min-h-[2.25rem] text-[1.1rem] font-bold leading-tight tracking-[-0.03em] sm:mt-2.5 sm:min-h-[2.55rem] sm:text-[1.18rem]",
            light ? "text-slate-900" : "text-slate-50"
          ].join(" ")}
        >
          {category.title}
        </h3>
        <p className={["mobile-clamp-2 mt-2 flex-1 text-[13px] leading-6 sm:mt-2.5 sm:text-[14px]", light ? "text-slate-600" : "text-slate-300/84"].join(" ")}>
          {category.description}
        </p>
        <div className={["mt-auto border-t pt-3 sm:mt-5 sm:pt-3.5", light ? "border-slate-200" : "border-sky-400/10"].join(" ")}>

      ...
      <Link href={`/${locale}${category.href}`} className="group block h-full">
        <GlassPanel className="ui-card ui-card-hover home-card-glow flex h-full flex-col overflow-hidden p-5 sm:p-6 md:p-7">
          <div className="flex min-h-[40px] items-start justify-between gap-3 overflow-hidden sm:min-h-[44px]">
            ...
            <Badge variant="muted" className="max-w-[88px] shrink-0 justify-center text-[11px]">
              {category.metric}
            </Badge>
          </div>
          <p className="mt-6 text-xs font-medium uppercase tracking-[0.18em] text-slate-400">{category.eyebrow}</p>
          <h3 className="clamp-2 mt-2 min-h-[2.65rem] text-[1.22rem] font-bold leading-tight tracking-[-0.03em] text-slate-50 sm:mt-3 sm:min-h-[3.15rem] sm:text-[1.42rem]">
            {category.title}
          </h3>

  ...
  return (
    <header
      className={[
        ...
        isScrolled
          ? "border-b border-slate-200/60 bg-[rgba(255,255,255,0.7)] shadow-[0_18px_46px_-34px_rgba(15,23,42,0.18)] backdrop-blur-3xl backdrop-saturate-150 supports-[backdrop-filter]:bg-[rgba(255,255,255,0.7)]"
          : "border-b border-slate-200/40 bg-[rgba(255,255,255,0.46)] shadow-[0_10px_28px_-34px_rgba(15,23,42,0.1)] backdrop-blur-2xl backdrop-saturate-150 supports-[backdrop-filter]:bg-[rgba(255,255,255,0.48)]"
      ].join(" ")}
    >
      <div className="w-full px-0 py-0">
        <div className="mx-auto flex w-full max-w-none flex-col gap-3 px-3 py-3 sm:px-4 lg:px-6">
          <div className="flex items-center gap-4">
            ...
            <nav className="flex min-w-0 flex-1 items-center justify-center gap-1.5 overflow-x-auto pb-0.5">
              {navItems.map((item) => (
                <Link
                  key={item.href}
                  href={`/${locale}${item.href}`}
                  className="inline-flex min-h-[40px] shrink-0 items-center rounded-full border border-slate-200/75 bg-white/48 px-4 text-[14px] font-semibold tracking-[-0.01em] text-slate-700 shadow-[0_8px_20px_-20px_rgba(15,23,42,0.18)] transition duration-200 hover:border-sky-300 hover:bg-sky-50 hover:text-[#0A2540] hover:shadow-[0_12px_26px_-18px_rgba(14,37,64,0.36)]"
                >
                  {item.label}
                </Link>
              ))}
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-0.5 lg:hidden">
            {navItems.map((item) => (
              <Link
                ...
                href={`/${locale}${item.href}`}
                className="inline-flex min-h-[36px] shrink-0 items-center rounded-full border border-slate-200/75 bg-white/48 px-3 text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-700 shadow-[0_8px_20px_-20px_rgba(15,23,42,0.18)] transition duration-200 hover:border-sky-300 hover:bg-sky-50 hover:text-[#0A2540] hover:shadow-[0_12px_26px_-18px_rgba(14,37,64,0.36)]"
              >
                {item.label}
              </Link>
            ))}
